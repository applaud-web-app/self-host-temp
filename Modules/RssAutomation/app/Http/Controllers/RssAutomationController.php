...
class RssAutomationController extends Controller
{
    ...
    {
        return view('rssautomation::create');
    }
    ...
    {
        try {
            // Validate: 'eq' is optional
            $request->validate([
                'eq' => 'nullable|string',
            ]);

            /* ---------------------------------------------------------------
            | Resolve optional RSS feed (?eq=...) WITHOUT using notification_id
            * ------------------------------------------------------------- */
            $feed  = null;
            $rssId = null;
            ...
            // Non-AJAX → render blade with optional $feed
            if (! $request->ajax()) {
                return view('rssautomation::report', compact('feed'));
            }

            /* ---------------------------------------------------------------
            | Base query (aligned with new "view" logic)
            * ------------------------------------------------------------- */
            $query = DB::table('notifications as n')
                ...
                ->where('n.segment_type', 'rss') // RSS-only report
                ->groupBy(
                    'n.id',
                    'n.campaign_name',
                    'n.schedule_type',
            ...
            /* ---------------------------------------------------------------
            | Filter to a specific feed — NO JOIN; use whereExists on timestamp
            | Matches rfn.last_sent_at == n.sent_at and rfn.rss_feed_id == $rssId
            * ------------------------------------------------------------- */
            if ($rssId) {
                $query->whereExists(function ($sub) use ($rssId) {
                    $sub->select(DB::raw(1))
                        ->from('rss_feed_notifications as rfn')
                        ->whereColumn('rfn.last_sent_at', 'n.sent_at')
                        ->where('rfn.rss_feed_id', $rssId);
                });

                // Optional (if exact timestamp equality is too strict):
                // $query->whereExists(function ($sub) use ($rssId) {
                //     $sub->select(DB::raw(1))
                //         ->from('rss_feed_notifications as rfn')
                //         ->where('rfn.rss_feed_id', $rssId)
                //         ->whereRaw('ABS(TIMESTAMPDIFF(SECOND, rfn.last_sent_at, n.sent_at)) <= 60');
                // });
            }

            /* ---------------------------------------------------------------
            | Dynamic filters (same shape as new "view" logic)
            * ------------------------------------------------------------- */
            $query->when($request->filled('status'),
                ...
                ->when($request->filled('campaign_type') && $request->campaign_type !== 'all', function ($q) use ($request) {
                    $type = $request->campaign_type;
                    $q->where(function ($sub) use ($type) {
                        $sub->where('n.schedule_type', $type)
                            ->orWhere('n.segment_type', $type);
                    });
                })
            ...
            return DataTables::of($query)
                ...
                ->addColumn('campaign_name', function ($row) {
                    ...
                    if (isset($segmentTypes[$row->segment_type]) && $row->segment_type !== 'all') {
                        $segment = '<small class="ms-1 text-secondary text-capitalize">['
                                . e($segmentTypes[$row->segment_type]) . ']</small>';
                    }
                    return '<div>' . e($row->campaign_name)
                        . ' <small class="ms-1 text-primary text-capitalize">[' . e($row->schedule_type) . ']</small>'
                        . $segment
                        . '<br><small> ' . e($truncated) . '</small></div>';
                })
                ->addColumn('status', function ($row) {
                    $map = [
                        'pending'   => ['badge-warning',   'Pending'],
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $html = '<button type="button" class="btn btn-primary light btn-sm report-btn rounded-pill"
                                data-bs-toggle="modal" data-bs-target="#reportModal" data-url="' . e($detailsUrl) . '">
                                <i class="fas fa-analytics"></i>
                            </button>';

                    $html .= ' <a href="' . e($cloneUrl) . '" class="btn btn-secondary light btn-sm mx-1 rounded-pill">
