...
namespace Modules\AdvanceAnalytics\Http\Middleware;
...
class CheckLicenseKey
{
    ...
    {
        try {
            $providedKey = decrypt(config('license.advance_analytics_key'));
            $providedName = config('license.advance_analytics_name');
