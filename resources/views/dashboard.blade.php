                                    ...
                                    <div class="media-body">
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <i class="fas fa-sync-alt notif-refresh refresh-icon"
                                                data-metric="{{ $key }}" title="Refresh"></i>
                                        </div>
        ...
        $(function() {
            ...
            $('#domain-select').on('select2:select', e => {
                ...
                fetchDomainStats().done(res => {
                    if (res.status) {
                        ...
                        iziToast.success({
                            ...
                            message: 'Subscriber stats refreshed',
                            position: 'topRight'
                        });
                    }
                });
                ...
                fetchNotificationStats().done(res => {
                    if (res.status) {
                        ...
                        iziToast.success({
                            ...
                            message: 'Notification stats refreshed',
                            position: 'topRight'
                        });
                    }
                });
                ...
                fetchWeeklyStats(false).done(res => {
                    if (res.status) {
                        ...
                        renderChart('weeklyNotificationsChart', labels, notifications,
                            'Notifications');
                    }
                    ...
                    .done(res => {
                        if (res.status) {
                            ...
                            iziToast.success({
                                ...
                                message: 'Subscriber stats updated',
                                position: 'topRight'
                            });
                        }
                    })
                    ...
                    .done(res => {
                        if (res.status) {
                            ...
                            iziToast.success({
                                ...
                                message: 'Notification stats updated',
                                position: 'topRight'
                            });
                        }
                    })
            ...
            $('.chart-refresh').click(function() {
                ...
                if (inFlightChart[metric]) {
                    return iziToast.warning({
                        title: 'Please wait',
                        message: 'Chart refresh in progress.',
                        position: 'topRight'
                    });
                }
                    ...
                    .done(res => {
                        if (res.status) {
                            ...
                            if (metric === 'subscribers' || !metric) {
                                renderChart('weeklySubscribersChart', labels, subscribers,
                                    'Subscribers');
                            }
            ...
            $('#refresh-all').click(() => {
                ...
                fetchDomainStats(true).done(res => {
                    if (res.status) {
                        ...
                        Object.entries(res.data).forEach(([k, v]) => {
                            animateValue($(`#card-${k}`)[0], 0, v,
                            1000); // Ensure proper reference to the element and value
                        });
                        iziToast.success({
                            title: 'Subscribers Refreshed',
                            message: 'Subscriber stats updated',
                ...
                fetchNotificationStats(true).done(res => {
                    if (res.status) {
                        console.log(res); // Debugging: Log the response
                        Object.entries(res.data).forEach(([k, v]) => {
                            animateValue($(`#notif-${k}`)[0], 0, v,
                            800); // Ensure proper reference to the element and value
                        });
                        iziToast.success({
                            title: 'Notifications Refreshed',
                            message: 'Notification stats updated',
